function [ cost, grad ] = stackedAECost(theta, layers, numClasses, ...
                                              lambda, data, labels)
...
% layers:      the stacked layers of the network
...
hiddenSize=layers(end);

softmaxTheta = reshape(theta(1:hiddenSize*numClasses), numClasses, hiddenSize);

layersize=length(layers)-1;
stack=cell(layersize,1);
pos = hiddenSize*numClasses;
for ii=1:layersize
    stack{ii}=struct;
    wlen = layers(ii) * layers(ii+1);
